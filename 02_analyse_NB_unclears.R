#!/usr/bin/env Rscript
#
# TRAIN data, and evaluate on 'unclears' (day 1)
#
################################################################################
...
# outdir 
outdir <- 'out.02.analyse.NB.unclears'
dir.create( outdir, showWarnings = FALSE )

# load 'unclear' data
df_unclear <- data.frame( readr::read_tsv( 'out.00.analyse.1A/df_unclear.tsv', show_col_types = FALSE ) )

### corpus aanmaken voor de unclear set ###
corpus_unclear <- corpus( df_unclear$fsc_letter_without_considerations_andEEG ) 
docvars( corpus_unclear, "fu_diagnosis_after_FU" ) <- df_unclear$fu_diagnosis_after_FU
summary( corpus_unclear )


### preprocessing op het corpus van de test set###
toks_unclear <- tokens( corpus_unclear, remove_punct = TRUE, remove_symbols = TRUE, 
                     remove_url = TRUE, remove_separators = TRUE ) %>% tokens_tolower()
toks_ngram_unclear <- tokens_ngrams( toks_unclear, n = 1:2 )
toks_clean_unclear <- tokens_remove( toks_ngram_unclear, stopwords )

###DFM aanmaken voor de test set###
dfm_unclear <- dfm( toks_clean_unclear )

###TF-IDF berekenen voor de test set###
dfm_unclear_tfidf <- dfm_tfidf( dfm_unclear )

### verder werken met de top 300 van de top 8000 features ###
dfm_train_tfidf_final <- dfm_select( dfm_train_tfidf, pattern = preds )
dfm_unclear_tfidf_final <- dfm_select( dfm_unclear_tfidf, pattern = preds )
dfm_unclear_tfidf_final <- dfm_match( dfm_unclear_tfidf_final, features = featnames( dfm_train_tfidf_final ) )
...
# add covariates
covariate1 <- df_unclear$p_sex
covariate2 <- df_unclear$p_age_fsle
...
# Make sure the number of rows matches your DFM
nrow( additional_covariates ) == nrow( dfm_unclear_tfidf_final )

# Combine DFM with covariates
dfm_unclear_combined <- cbind( dfm_unclear_tfidf_final, dfm_covariates )



#check werkelijke calssificatie
actual_class_unclear <- docvars( dfm_unclear_combined, "fu_diagnosis_after_FU" )

#predict class
predicted_class_unclear <- predict( nb_model, newdata = dfm_unclear_combined )

#2x2 tabel
tab_class_unclear <- table( actual_class_unclear, predicted_class_unclear )
...
readr::write_tsv( perf_unclear, file = paste0( outdir, '/performance_nb_bayes__df_unclear.tsv' ), quote = 'all' )
