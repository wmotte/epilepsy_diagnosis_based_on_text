...
# outdir
outdir <- 'out.04.xgboost'
dir.create( outdir, showWarnings = FALSE )

# read input data
df <- read.csv( 'colab_embedding/embedded_df.csv', row.names = 1 )
...
# train Area under the curve: 0.7502 [5, 0.07, 2]
model <- xgboost( data = data, label = label, max.depth = 5, eta = 0.07, nrounds = 2, 
                  nthread = 2, 
